...
{(
  ...
  SDK_VER_FOLDER_PATH="$RAMDISK_PATH/rootfs/overlay/${CVI_ARCH_LOWER}_${SDK_VER}"
...
{(
  ...
  python3 "$IMGTOOL_PATH"/mk_package.py "$FLASH_PARTITION_XML" "$OUTPUT_DIR" -o "$OUTPUT_DIR"/${CHIP}-${ARCH}-$(LC_ALL=C date +%F)-${FILE_SYSTEM}-${STORAGE_TYPE}.zip $extra_files_args
...
{(
  ...
  cd usb_dl
  unzip -o $OUTPUT_DIR/${CHIP}-${ARCH}-$(LC_ALL=C date +%F)-${FILE_SYSTEM}-${STORAGE_TYPE}.zip
