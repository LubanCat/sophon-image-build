...
int cv2003_1l_slave1_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunCV2003_1L_SLAVE1_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, cv2003_1l_slave1_i2c_addr);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_SLAVE1_FORCE error!\n");
		close(g_fd[ViPipe]);
		g_fd[ViPipe] = -1;
		return ret;
	}
...
int cv2003_1l_slave1_read_register(VI_PIPE ViPipe, int addr)
{
	...
	if (cv2003_1l_slave1_addr_byte == 2)
		buf[idx++] = (addr >> 8) & 0xff;

	// add address byte 0
	buf[idx++] = addr & 0xff;

	ret = write(g_fd[ViPipe], buf, cv2003_1l_slave1_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, cv2003_1l_slave1_data_byte);
	if (ret < 0) {
	...
	// pack read back data
	data = 0;
	if (cv2003_1l_slave1_data_byte == 2) {
		data = buf[0] << 8;
		data += buf[1];
	} else {
		data = buf[0];
...
int cv2003_1l_slave1_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	if (cv2003_1l_slave1_data_byte == 1) {
		buf[idx] = data & 0xff;
		idx++;
	}

	ret = write(g_fd[ViPipe], buf, cv2003_1l_slave1_addr_byte + cv2003_1l_slave1_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_WRITE error!\n");
		return CVI_FAILURE;
	}
	ret = read(g_fd[ViPipe], buf, cv2003_1l_slave1_addr_byte + cv2003_1l_slave1_data_byte);
